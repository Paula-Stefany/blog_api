        ...
        try:
            with app.conexao_bd.cursor(dictionary=True) as cursor:
                ...
                if id_usuario is None or tipo_usuario is None:
                    return jsonify({"mensagem": "ceredenciais inválidas"})
    ...
    def cadastro_novo_autor():
        ...
        try:
            with app.conexao_bd.cursor(dictionary=True) as cursor:
                ...
                if not novo_autor:
                    return {'mensagem': 'voce precisa digitar seus dados'}

                if novo_autor['email'] != email:
                    return jsonify({'mensagem': 'digite o email que voce recebeu o link de convite!'})

                email_presente = verificar_se_email_autor_ja_existe_no_banco(email, cursor)
                if email_presente:
                    return jsonify({'mensagem': 'Autor com esse email já cadastrado'})
                ...
                return jsonify({"mensagem": "autor inserido com sucesso"}), 200
    ...
    def registrar_novo_admin():
        ...
        try:
            with app.conexao_bd.cursor(dictionary=True) as cursor:
                ...
                if not novo_admin:
                    return jsonify({'mensagem': 'Voce precisa digitar seus dados'})

                if novo_admin['email'] != email:
                    return jsonify({'mensagem': 'digite o email que você recebeu o link de convite.'})

                email_presente = verificar_se_email_admin_ja_existe_no_banco(email, cursor)
                if email_presente:
                    return jsonify({'mensagem': 'administrador com esse email já cadastrado'})
                ...
                return jsonify({'mensagem': 'Novo administrador cadastrado com sucesso!'})
