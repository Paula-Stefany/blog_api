    ...
    def obter_autores():
        ...
        try:
            ...
            with app.conexao_bd.cursor(dictionary=True) as cursor:
                ...
                if not autores:
                    return jsonify({"mensagem": "nenhum autor encontrado"})
    ...
    def editar_autor(id_autor, perfil, nome_autor):
        ...
        try:
            with app.conexao_bd.cursor(dictionary=True) as cursor:
                ...
                if not autor:
                    return jsonify({"mensagem": "dados inválidos, voce só pode editar seus próprios dados"})

                modificacoes = request.get_json()
                if not modificacoes:
                    return jsonify({"mensagem": "Você precisa digitar seus dados"})
                ...
                return jsonify({'mensagem': 'auteração feita com sucesso'})
